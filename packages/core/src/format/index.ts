/**
 * Formatting money and dates for a Dutch reader.
 *
 * A **browser-safe** subpath of `@klopt/core`, imported as `@klopt/core/format`.
 * The package's main entry reads reference data with `node:fs` and hashes with
 * `node:crypto`, so a bundler that follows it into a browser build produces a
 * module that throws on first use. This file imports nothing at all.
 *
 * It exists as one module rather than two because there are exactly two
 * consumers and they must agree: the screens, and the PDF rendering of an
 * invoice. A figure that reads 1.210,00 on screen and 1210.00 on the document
 * the customer receives is a support call.
 *
 * Nothing here divides by 100 in a float. Not to display, not to preview, not
 * ever: the moment a figure passes through a float the number on the page and
 * the number in the ledger are two different numbers.
 */
...
/** Integer division and string surgery only. */
 ...
 *
 * The same arithmetic `priceInvoice` does, exposed here because the browser
 * needs it for a preview and cannot import the pricing module. `Number(price) *
 * quantity` disagrees with it for exactly the amounts people invoice.
 */
