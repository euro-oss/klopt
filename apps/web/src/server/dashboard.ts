...
import { hasPermission } from '~/api/context'
import { listPurchaseInvoicesQuery } from '~/api/schemas'
import { buildWorkQueue } from '~/lib/work-queue'
import { isFiscalYearCode, withinFiscalYear } from '~/lib/fiscal-year'
 ...
 * about several subsystems at once, and asking it as five round trips would
 * mean the year has to be resolved before the first of them can be sent. The
 * reads are the same handlers `/api/v1` serves; nothing new is modelled here
 ...
 *
 * A row the reader may not act on is left out rather than shown and refused:
 * an auditor holds read and export, and telling them four invoices need
 * approving is telling them about somebody else's afternoon.
 *
...
export const getWorkQueue = createServerFn({ method: 'GET' })
  .validator((input: { fiscalYear?: string } | undefined) => input ?? {})
  .handler(async ({ data }) =>
    run(async () => {
      const context = await contextFromRequest()
      const { scope } = await reportYear(
        context,
        isFiscalYearCode(data.fiscalYear) ? data.fiscalYear : null,
      )

      const [drafts, overdue, dunning, transactions, inbox, purchases] = await Promise.all([
        handleListInvoices(context, { status: 'draft', limit: 500, updatedSince: null }),
        handleListOverdueInvoices(context, { asOf: scope.asOf }),
        handleGetDunningQueue(context, { asOf: scope.asOf }),
      ...
      const items = buildWorkQueue(
        {
          ...
          salesOverdueTotal: overdueInYear
            ...
            .toString(),
          // A reminder that is due and has not been sent. Not the same as
          // "overdue": an invoice a week late has a reminder waiting, and the
          // same invoice after the letter went out has not.
          dunningWaiting: dunning.body.actions.filter(
            (action) => action.sendable && withinFiscalYear(scope, action.dueDate),
          ).length,
          ...
          purchaseToApprove: purchasesInYear.filter((invoice) => invoice.status === 'booked')
            .length,
        },
